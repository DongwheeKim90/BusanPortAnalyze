...
# Case 1
if selectData == ":rainbow[Time series data on shipping companies/cargo volumes by port in South Korea]":
    st.markdown(
        """
...
# Case 2
elif selectData == ":rainbow[Time series data on shipping companies and cargo volumes entering the three major ports of Busan, South Korea]":
    st.markdown(
        """
        <div style="background-color:#cce5ff; padding:10px; border-radius:5px;">
            <span style="color:#ff8c00; font-weight:bold;">
...
# Case 3
elif selectData == ":rainbow[Annual sales amount and number of sales data for pre-sale items]":
    st.markdown(
        """
        <div style="background-color:#cce5ff; padding:10px; border-radius:5px;">
            <span style="color:#ff8c00; font-weight:bold;">
    ...
    with dataArea:
        with st.container(height=450):
            readData = pd.read_csv("./useData/forLLM_data/prod(YearMonth)_countPrice.csv", encoding="utf-8-sig")
            st.dataframe(readData, use_container_width=True, hide_index=True)
